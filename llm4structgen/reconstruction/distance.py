...
class Distances:
    ...
    def get_features(
        ...
        return self.get_distances(positions, cell, offset_count, device)
...
class PBC_offsets:
    def __init__(self, cell, device, supercell_max=4):
        ...
        for offset_num in range(0, supercell_max):
            ...
            unit_cell = torch.tensor(unit_cell, dtype=torch.float, device=device)
            self._pbc_offsets.append(torch.mm(unit_cell, cell.to(device)))

    def get_offset(self, offset_num):
        return self._pbc_offsets[offset_num]